...
while True:
    avail_idx = []
    for point in start_point.next_points:
        avail_idx.append(point.idx)
    A_idx = int(input('Please enter the next vertex that A moves to, the available vertices are:' + str(avail_idx)) + '\n')
    while A_idx not in avail_idx or not isinstance(A_idx, int):
        A_idx = int(input('Input error, please re-enter the next vertex that A moves to, the available vertices are:' + str(avail_idx)) + '\n')
    A = eval('point_' + str(A_idx))
    ...
    if A.idx == point_4.idx:
        ...
        for point in A.next_points:
            avail_idx.append(point.idx)
        E_idx = int(input('The current vertice is:' + str(A.idx) + ', Please enter the next vertex that E moves to. The available vertices are:' + str(avail_idx)) + '\n')
